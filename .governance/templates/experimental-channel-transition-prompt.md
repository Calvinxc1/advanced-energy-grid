...
Transition downstream workspaces only from the selected target branch's `.governance/` tree. Never use this repository's `.governance/` tree as the source or an external reference for another workspace. Do not copy the target branch's concrete `.governance/status.yaml`; create or update downstream status from `.governance/templates/local-status.yaml`.
...
- read `AGENTS.md`
- read `.governance/status.yaml` if present
...
3. Read `governance-bootstrap-prompt.md` from literal branch `main`, then read the target branch's `AGENTS.md`, `.governance/branch-descriptor.yaml`, and `.governance/governance-init.md`.
4. Copy or merge the target branch's `.governance/` product into this workspace, excluding the target branch's concrete `.governance/status.yaml`.
5. Preserve workspace-local files under `.governance/local/` unless Jason or explicit policy says to reconcile them.
6. Ensure `.governance/local/index.yaml` exists, using `.governance/templates/local-index.yaml` from the target branch when needed.
7. Create or update this workspace's `.governance/status.yaml` from `.governance/templates/local-status.yaml` so it records the target branch, target active canon version, target generalized canon version, source repository, source branch, source commit when known, verification date, and any unresolved reconciliation notes.
