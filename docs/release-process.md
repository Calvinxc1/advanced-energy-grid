...
- `feature/*` branches are short-lived implementation or parked-work branches created from `dev` and merged back to `dev`.
...
Use this feature branch shape for ordinary scoped implementation:
...
```text
Calvinxc1/advanced-energy-grid
```
...
```text
GITHUB_RELEASE_REPOSITORY=Calvinxc1/advanced-energy-grid
```
