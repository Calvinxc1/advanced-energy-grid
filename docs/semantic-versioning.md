...
Advanced Energy Grid uses Semantic Versioning in `MAJOR.MINOR.PATCH` form, with the additional pre-`1.0.0` rules below for gameplay and prototype changes.
...
- Adding a new pole, substation, recipe, technology, or compatibility path.
- Changing progression numbers such as wire reach, supply area, recipe cost, prerequisites, or unlock timing.
...
- Release zip files use `{mod-name}_{version}.zip`, for example `advanced-energy-grid_0.1.0.zip`.
